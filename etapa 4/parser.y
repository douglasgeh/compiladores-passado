...
nome_global: TK_IDENTIFICADOR {
        scopesManager.insertVariableWithUndefinedType($1->numeroLinha, 0, $1);
    }
...
literal: TK_LIT_INT { 
        ...
        scopesManager.insertLiteral(get_line_number(), 0, $1, intSType);
    }
    | TK_LIT_FLOAT { 
        $$ = createNodeNoTypeWithSType($1, floatSType);
        scopesManager.insertLiteral(get_line_number(), 0, $1, floatSType);    
    }
    | TK_LIT_FALSE { 
        $$ = createNodeNoTypeWithSType($1, boolSType);
        scopesManager.insertLiteral(get_line_number(), 0, $1, boolSType);
    }
    | TK_LIT_TRUE { 
        $$ = createNodeNoTypeWithSType($1, boolSType);
        scopesManager.insertLiteral(get_line_number(), 0, $1, boolSType);
    ...
    | TK_LIT_CHAR{
        ...
        scopesManager.insertLiteral(get_line_number(), 0, $1, charSType);
    };

funcao: tipo TK_IDENTIFICADOR inicia_escopo '(' lista_argumentos_func ')' corpo {
    AST* nodeFuncDec = createNodeWithLexicalTypeAndValue(functionType, $2);
    appendChild(nodeFuncDec, $7);
    $$ = nodeFuncDec;
 //   scopesManager.insertFunction($2->numeroLinha, 0, nodeFuncDec, (SyntacticalType) $1);
...
cmdDecVariavelLocalInicializacao: TK_IDENTIFICADOR TK_OC_LE literal { 
        ...
        AST *identifierNode = createNodeNoType($1);
